...
export default function PendingApprovalBanner({ spaceId, inviteToken }: { spaceId: string, inviteToken?: string | null }) {
  ...
  return (
    <div style={{ position: 'relative', background: currentMember.status === 'disputed' || currentMember.status === 'extension_requested' ? '#fef3c7' : '#eff6ff', border: currentMember.status === 'disputed' || currentMember.status === 'extension_requested' ? '1px solid #f59e0b' : '1px solid #3b82f6', padding: '1.25rem', borderRadius: '16px', marginBottom: '1.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      ...
      {showRegisterPrompt && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>
          <div style={{ background: 'white', width: '90%', maxWidth: '420px', borderRadius: '24px', padding: '2.5rem 2rem', textAlign: 'center', animation: 'scaleIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)', boxShadow: '0 20px 40px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ fontSize: '3.5rem', marginBottom: '0.5rem' }}>👋</div>
            <h3 style={{ margin: 0, color: '#0f172a', fontSize: '1.75rem', fontWeight: '800' }}>ברוך הבא!</h3>
            <p style={{ color: '#475569', margin: '0 0 1.5rem 0', lineHeight: 1.6, fontSize: '1.05rem' }}>
              כדי להיכנס למרחב בצורה מאובטחת ולשמור את הנתונים שלך, אנא התחבר.
            </p>
            ...
            <button 
              onClick={async () => {
                try {
                  ...
                  // Error handled in AuthContext (throws so we don't proceed)
                }
              }}
              style={{ width: '100%', background: '#4285F4', color: 'white', border: 'none', padding: '1.2rem', borderRadius: '16px', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', boxShadow: '0 4px 12px rgba(66,133,244,0.3)', transition: 'transform 0.2s' }}
            >
              <span style={{ background: 'white', borderRadius: '50%', padding: '0.2rem', display: 'flex' }}>
