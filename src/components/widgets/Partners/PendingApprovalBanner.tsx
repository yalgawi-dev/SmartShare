    ...
    <div style={{ position: 'relative', background: currentMember.status === 'disputed' || currentMember.status === 'extension_requested' ? '#fef3c7' : '#eff6ff', border: currentMember.status === 'disputed' || currentMember.status === 'extension_requested' ? '1px solid #f59e0b' : '1px solid #3b82f6', padding: '1.25rem', borderRadius: '16px', marginBottom: '1.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      ...
      {showRegisterPrompt && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'white', width: '90%', maxWidth: '400px', borderRadius: '24px', padding: '2rem', textAlign: 'center', animation: 'scaleIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>👋</div>
            <h3 style={{ margin: '0 0 1rem 0', color: '#0f172a', fontSize: '1.5rem' }}>רגע! בוא נכיר</h3>
            <p style={{ color: '#475569', marginBottom: '2rem', lineHeight: 1.6, fontSize: '1.05rem' }}>
              כדי לאשר את השותפות, המערכת חייבת לדעת מי אתה. התחבר בצורה מאובטחת עם גוגל (בקליק אחד).
            </p>
            <button 
              onClick={async () => {
                try {
              ...
              style={{ width: '100%', background: 'var(--primary)', color: 'white', border: 'none', padding: '1rem', borderRadius: '16px', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer', marginBottom: '1rem', boxShadow: '0 4px 12px rgba(74,91,240,0.2)' }}
            >
              התחבר עם Google
            </button>
            <button 
              onClick={() => setShowRegisterPrompt(false)}
              style={{ background: 'transparent', border: 'none', color: '#94a3b8', fontWeight: 'bold', cursor: 'pointer', padding: '0.5rem', fontSize: '1rem' }}
            >
              בטל וחזור לאחור
            </button>
          </div>
