          ...
          {ocrData?._duplicateWarning && (
            <div style={{ padding: '0.75rem 1rem', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '12px', color: '#dc2626', fontSize: '0.9rem', display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
              ...
              <div>
                ...
                {ocrData._duplicateInvoice && (
                  <button type="button" onClick={(e) => { 
  e.preventDefault(); 
  const url = ocrData._duplicateInvoice.attachmentUrl || ocrData._duplicateInvoice.imageUrl;
  if (url) { window.open(url, '_blank'); } else { alert('לחשבונית המקורית אין תמונה סרוקה, אך היא קיימת בטבלת ההוצאות.'); }
}} style={{ marginTop: '0.75rem', background: '#fee2e2', border: '1px solid #f87171', color: '#b91c1c', padding: '0.5rem 0.8rem', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.4rem', width: 'fit-content' }}>
