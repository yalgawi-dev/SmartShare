...
text = text.replace("הגדרת חלוקת אחוזים (v1.1)", "הגדרת חלוקת אחוזים (v1.4)");

const start = text.indexOf("{isPending && (");
const end = text.indexOf(")}", start + 300) + 2;
...
if (start !== -1 && end !== -1) {
    const endOfBlock = text.indexOf(")}", text.indexOf("</div>", start)) + 2; 
    text = text.substring(0, start) + newBlock + text.substring(endOfBlock);
