...
const cleanStart = `        <div 
          onClick={() => setShowTotalBreakdown(true)}`;
const cleanEnd = `      <div style={{ marginBottom: '1.5rem' }}>`;

const idx = content.indexOf(cleanStart);
if (idx !== -1) {
    const endIdx = content.indexOf(cleanEnd, idx);
    if (endIdx !== -1) {
...
`;
        ...
        console.log("Rewrote entire cubes section cleanly.");
    } else { console.log("cleanEnd not found"); }
} else { console.log("cleanStart not found"); }
