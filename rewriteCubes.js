...
// We'll replace the ENTIRE first row of cubes to be absolutely sure there's no syntax error.
const startToken = "      {/* Overview Cards */}\n      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', overflowX: 'auto', paddingBottom: '0.5rem', scrollbarWidth: 'none' }}>";

const idx = content.indexOf(startToken);
if (idx !== -1) {
    const endToken = "      <div style={{ marginBottom: '1.5rem' }}>";
    const endIdx = content.indexOf(endToken, idx);
    
    if (endIdx !== -1) {
        const replacement = `      {/* Overview Cards */}
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', overflowX: 'auto', paddingBottom: '0.5rem', scrollbarWidth: 'none' }}>
        <div 
